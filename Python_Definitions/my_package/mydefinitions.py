...
def do_fft(data, time_interval_between_points, data_title, return_values=False):
    ...
    frequencies = np.fft.fftfreq(len(processed), d=time_interval_between_points)[
        : len(processed) // 2
    ]
    ...
    fig.update_layout(
        title=f"Fourier Transform {data_title}", xaxis_title="Frequency [Hz]"
    )
...
def read_data(
    ...
    all_spectra_list = (
        []
    )  # empty list to store spectra, where one spectrum is a list of values at different wavelengths
    indices_for_files_list = (
        []
    )  # for storing index at which given spectrum was obtained (for example angles or xy)
    ...
    for file in glob.glob(folder_path):
        ...
        if search_phrase in file and exclude_phrase not in file:
            ...
            index_for_file = float(
                file.replace(part_to_remove1, "").replace(part_to_remove2, "")
            )
            ...
            data_from_file = np.loadtxt(
                file, delimiter=";", skiprows=skip_rows_1, unpack=True
            )
    ...
    all_data = pd.DataFrame(
        data=all_spectra_list, index=indices_for_files_array, columns=wavelengths_array
    )
...
def read_powermeter_csv(
    ...
    for file in glob.glob(folder_path):
        ...
        if search_phrase in file and exclude_phrase not in file:
            ...
            # Sometimes 167 measurements happen, so only first 166 are taken
    ...
    # saving data in table format
    all_data = pd.DataFrame(
        data=powermeter_readings, index=indices_for_files_array, columns=time_array
    )
    all_data = all_data.sort_index()
...
def plotting_polarization(wavelengths_list, dataframe, no_polarizer_dataframe=None):
    ...
    if no_polarizer_dataframe is not None:
        for wv_exact in wv_list:
            ax2.plot(
                dataframe[wv_exact] / no_polarizer_dataframe.loc[1][wv_exact],
                ".-",
                label=f"{wv_exact}",
            )
        ...
        for wv_exact in wv_list:
            ax2.plot(
                dataframe[wv_exact] / np.mean(dataframe[wv_exact]),
                ".-",
                label=f"{wv_exact}",
            )
...
def eliminate_noisy_columns(dataframe, noise_array, SN_ratio=None):
    ...
    first_array = eliminate_noisy_data(
        dataframe.loc[0].values, noise_array, SN_ratio_value=SN_R
    )

    for index in dataframe.index.values[1:]:
        new_array = eliminate_noisy_data(
            dataframe.loc[index].values, noise_array, SN_ratio_value=SN_R
        )
...
def fit_gaussian_to_list(
    ...
    bins_centres = np.array(
        [0.5 * (bins_edges[i] + bins_edges[i + 1]) for i in range(len(bins_edges) - 1)]
    )
    ...
    params, params_covariance = curve_fit(
        gaussian, bins_centres, counts, p0=initial_guesses, maxfev=maxfev_value
    )
...
def fit_linear_to_data(
    ...
    plt.errorbar(
        x_data, y_data, xerr=x_err, yerr=y_err, fmt="o", capsize=5, label="Data"
    )
